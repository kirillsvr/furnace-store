				...
				if (strlen($atom_data) > 0) {
					if ((strlen($atom_data) % $GPS_rowsize) == 0) {
						...
						foreach ($atom_structure['gps_toc'] as $key => $gps_pointer) {
							...
							/*
							// 2021-05-10: I see some of the data, notably the Hour-Minute-Second, but cannot reconcile the rest of the data. However, the NMEA "GPRMC" line is there and relatively easy to parse, so I'm using that instead
...
echo 'QuicktimeParseNikonNCTG()::unknown $data_size_type: '.$data_size_type.'<br>';
	...
	// code by Steffen Hartmann 2021-Nov-08
	*/
