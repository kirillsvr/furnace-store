       ...
       jQuery.each(woof_qt_current_values.add_filter, function (key_tax,tax){
           ...
           jQuery.each(tax,function (key,term_id){
               if(term_id || term_id>0){
                   ...
                   //sql_tax.push(' d.term_ids REGEXP  "[[:<:]]34[[:>:]]" ');
               } 
           })
           if(sql_tax.length){
           ...
           //console.log(sql_tax.join(term_logic))
           }
       });
...
function woof_qt_get_template(){
   ...
   // var item_tpl=jQuery(woof_qt_tpl_item_selector)[0].outerHTML;
   //var no_product_tpl=jQuery(woof_qt_tpl_no_product_selector)[0].outerHTML;
...
function woof_init_qt_pagination(){
   jQuery('.woof_qt_pagination_item').on('click',function(){
      ...
      // jQuery(this).addClass('qt_current');
...
function woof_qt_init_ion_sliders() {
	...
	try {
		...
		onFinish: function (ui) {
		    //woof_qt_current_values.min_price = parseInt(ui.from, 10);
		   // woof_qt_current_values.max_price = parseInt(ui.to, 10);
...
function woof_qt_init_checkbox(){
    if (icheck_skin != 'none') {
        ...
        jQuery('.woof_qt_checkbox').unbind('ifChecked');
        ...
        jQuery('.woof_qt_checkbox').unbind('ifUnchecked');
        ...
        jQuery('.woof_qt_checkbox_label').unbind();
        jQuery('label.woof_qt_checkbox_label').click(function () {
...
function woof_qt_init_radio(){
        ...
        jQuery('.woof_qt_radio').unbind('ifChecked');
    ...
    jQuery('.woof_qt_radio_reset').click(function () {
        var slug=jQuery(this).attr('data-tax');
